 ...
 * request, for example in the case of GC. The state machine uses the following
 * states:
 * <ul>
 * <li>NEW</li>
 * <li>IN_JAVA</li>
 ...
 * <ul>
 * <li>NEW to IN_JAVA: occurs when the thread is actually started. At this point
 * it is safe to expect that the thread will reach a safe point in some bounded
 * amount of time, at which point it will have a complete execution context, and
 * this will be able to have its stack traces by GC.</li>
 * <li>IN_JAVA to IN_JAVA_TO_BLOCK: occurs when an asynchronous request is made,
 * for example to stop for GC, do a mutator flush, or do an isync on PPC.</li>
 * <li>IN_JAVA to IN_NATIVE: occurs when the code opts to run in privileged
 * mode, without synchronizing with GC. This state transition is only performed
 * by HeavyCondLock, in cases where the thread is about to go idle while waiting
 * for notifications (such as in the case of park, wait, or sleep).</li>
 * <li>IN_JAVA to IN_JNI: occurs in response to a JNI downcall, or return from a
 * JNI upcall.</li>
 ...
 * any user Java code, and will not be able to return to running Java code
 * without first blocking, and waiting to be unblocked (see BLOCKED_IN_NATIVE to
 * IN_JAVA transition.</li>
 ...
 * go back to running Java code. The actual transition is preceded by the thread
 * first performing any requested actions (such as mutator flushes) and waiting
 * for a notification that it is safe to continue running (for example, the
 * thread may wait until GC is finished).</li>
 * <li>IN_JNI to IN_JAVA: occurs when a thread returns from a JNI downcall, or
 * makes a JNI upcall.</li>
 * <li>BLOCKED_IN_JNI to IN_JAVA: same as BLOCKED_IN_NATIVE to IN_JAVA, except
 * that this occurs in response to a return from a JNI downcall, or as the
 * thread makes a JNI upcall.</li>
 * <li>IN_JAVA to TERMINATED: the thread has terminated, and will never reach
 * any more safe points, and thus will not be able to respond to any more
 * requests for async activities.</li>
 * </ul>
 * Observe that the transitions from BLOCKED_IN_NATIVE and BLOCKED_IN_JNI to
 * IN_JAVA constitute a safe point. Code running in BLOCKED_IN_NATIVE or
 * BLOCKED_IN_JNI is "GC safe" but is not quite at a safe point; safe points are
 * special in that they allow the thread to perform async activities (such as
 * mutator flushes or isyncs), while GC safe code will not necessarily perform
 * either.
 *
...
public final class RVMThread extends ThreadContext implements Constants {
	...
	protected static final boolean dumpStackOnBlock = false; // DANGEROUS! can
																// lead to
																// crashes!
	 ...
	 * thread death with for example MMTk. For production, this should never be
	 * set to true.
	 */
	 ...
	 * state, similar to the IN_JNI state but requiring different interaction
	 * with the collector (as there is no JNI stack frame, the registers have to
	 * be saved in contextRegisters). As well, this state should only be entered
	...
	/**
	 * thread is in Java code but is expected to block. the transition from
	 * IN_JAVA to IN_jAVA_TO_BLOCK happens as a result of an asynchronous call
	 * by the GC or any other internal VM code that requires this thread to
	 * perform an asynchronous activity (another example is the request to do an
	 * isync on PPC). the point is that we're waiting for the thread to reach a
	 * safe point and expect this to happen in bounded time; but if the thread
	 * were to escape to native we want to know about it. thus, transitions into
	 * native code while in the IN_JAVA_TO_BLOCK state result in a notification
	 * (broadcast on the thread's monitor) and a state change to
	 * BLOCKED_IN_NATIVE. Observe that it is always safe to conservatively
	 * change IN_JAVA to IN_JAVA_TO_BLOCK.
	 */
	public static final int IN_JAVA_TO_BLOCK = 4;

	/**
	 * thread is in native code, and is to block before returning to Java code.
	 * the transition from IN_NATIVE to BLOCKED_IN_NATIVE happens as a result of
	 * an asynchronous call by the GC or any other internal VM code that
	...
	/**
	 * @return the callee frame pointer for the stack frame hijacked by the
	 *         return barrier
	 */
	...
	private boolean attemptFastExecStatusTransition(int oldState, int newState) {
		if (Synchronization.tryCompareAndSwap(this, Entrypoints.execStatusField.getOffset(), oldState, newState)) {
	 ...
	 * that this field is not set atomically with the entering of the thread
	 * onto the aboutToTerminate array - in fact it happens before that. When
	 * this field is set to true, the thread's stack will no longer be scanned
	 * by GC. Note that this is distinct from the TERMINATED state.
	 */
	...
	/**
	 * Is the thread no longer executing user code? Protected by the Java
	 * monitor associated with the Thread object.
	 */
	boolean isJoinable;

	/**
	 * Link pointer for queues (specifically ThreadQueue). A thread can only be
	 * on one such queue at a time. The queue that a thread is on is indicated
	 * by <code>queuedOn</code>.
	 */
	@Untraced
	RVMThread next;
	...
	/**
	 * Index of this thread in {@link #threadBySlot}[]. This value must be
	 * non-zero because it is shifted and used in {@link Object} lock ownership
	 * tests.
	 */
	@Entrypoint
	...
	/**
	 * Non-null indicates this is a system thread, that is one used by the
	 * system and as such doesn't have a Runnable...
	 */
	 ...
	 * processor-specific counter so that it can be updated efficiently on
	 * SMP's.
	 */
	 ...
	 * A thread remains blocked so long as any of the block adapters say that it
	 * should be blocked. Block adapters are statically allocated, and store
	 * their state in instance fields of RVMThread.
	 */
	@Uninterruptible
	@NonMoving
	public abstract static class BlockAdapter {
		/**
		 * Should the given thread be blocked for this block adapter? If this
		 * returns true, the thread is guaranteed to block.
		 */
		abstract boolean isBlocked(RVMThread t);

		/**
		 * Specify that the thread is either blocked (value == true) or not
		 * blocked (value == false) for this block adapter. This call indicates
		 * a statement of fact by the thread itself - it's used either to
		 * acknowledge a block request (see hasBlockRequest below) or to respond
		 * to a request to unblock.
		 */
		abstract void setBlocked(RVMThread t, boolean value);

		/**
		 * Request that the thread block, for this block adapter, at its
		 * earliest convenience. Called from RVMThread.block() and associated
		 * methods. Some block adapters allow for multiple requests to block; in
		 * that case this will return a "token" that can be passed to
		 * hasBlockRequest() to check, not only whether there is a block
		 * request, but whether that block request is still associated with a
		 * particular call to requestBlock(). This is used to prevent a
		 * suspend() call from stalling due to a concurrent resume() and second
		 * suspend(). Note that most block adapers don't care about this
		 * scenario, and will just return 0 (or some other meaningless number)
		 * here.
		 */
		...
		/**
		 * Does the thread have a request to block associated with the given
		 * requestBlock() call?
		 */
	...
	static final BlockAdapter[] blockAdapters = new BlockAdapter[] { suspendBlockAdapter, handshakeBlockAdapter,
			gcBlockAdapter };
	 ...
	 * some Java program that queries the thread state will get something other
	 * than what it may or may not have expected.
	 */
	...
	/**
	 * Should the next executed yieldpoint be taken? Can be true for a variety
	 * of reasons. See RVMThread.yieldpoint
	 * <p>
	 * To support efficient sampling of only prologue/epilogues we also encode
	 * some extra information into this field. 0 means that the yieldpoint
	 * should not be taken. >0 means that the next yieldpoint of any type should
	 * be taken <0 means that the next prologue/epilogue yieldpoint should be
	 * taken
	 * <p>
	 ...
	 * ensures that the yieldpoint is deferred by setting
	...
	/**
	 * Thread handle. Currently stores pthread_t, which we assume to be no
	 * larger than a pointer-sized word.
	 */
	...
	/**
	 * Is the system in the process of shutting down (has System.exit been
	 * called)
	 */
	 ...
	 * thus release it. Someone else might then acquire the lock before
	 * realizing that there is a GC request and then do bad things.</li>
	 ...
	 * accounting. This mutex should not be held while thread monitors (see
	 * monitorBySlot) are held. Use this mutex only to protect accesses to:
	 * <ul>
	 * <li>the global thread lists, such as threadBySlot, aboutToTerminate,
	 * threads, and freeLots</li>
	 * <li>threadIdx field of RVMThread</li>
	 * <li>numThreads, numActiveThreads, numActiveDaemons static fields of
	 * RVMThread</li>
	 * </ul>
	 */
	...
	/**
	 * Thread slots of threads that are about to terminate. This must be an int
	 * array because it's accessed from code that cannot have barriers.
	 */
	 ...
	 * to numThreads-1 (inclusive) are defined. Note that it should be possible
	 * to scan this array without locking and get all of the threads - but only
	 * if you scan downward and place a memory fence between loads.
	 * <p>
	...
	public void assertAcceptableStates(int expected1, int expected2) {
		if (VM.VerifyAssertions) {
			int curStatus = getExecStatus();
			if (curStatus != expected1 && curStatus != expected2) {
				VM.sysWriteln("FATAL ERROR: unexpected thread state.");
				VM.sysWriteln("Expected: ", expected1);
				VM.sysWriteln("      or: ", expected2);
				VM.sysWriteln("Observed: ", curStatus);
				VM._assert(curStatus == expected1 || curStatus == expected2);
			}
		}
	}
	...
	public void assertUnacceptableStates(int unexpected1, int unexpected2) {
		if (VM.VerifyAssertions) {
			int curStatus = getExecStatus();
			if (curStatus == unexpected1 || curStatus == unexpected2) {
				...
				VM.sysWriteln("  Observed: ", curStatus);
				VM._assert(curStatus != unexpected1 && curStatus != unexpected2);
			}
		}
	}
	...
	static void bind(int cpuId) {
		if (VM.VerifyAssertions)
			VM._assert(sysCall.sysThreadBindSupported() == 1);
	...
	public static void boot() {
		...
		if (traceAcct)
			VM.sysWriteln("RVMThread booted");
	}

	/**
	 * Add this thread to the termination watchlist. Called by terminating
	 * threads before they finish terminating.
	 */
	...
	/**
	 * Method called after processing a list of threads, or before starting a
	 * new thread. This does two things. First, it guarantees that the thread
	 * slots used by any dead threads are freed. Second, it guarantees that each
	 * thread deregisters itself from GC. Thus, it is essential that after
	 * requesting things like mutator flushes, you call this, to ensure that any
	 * threads that had died before or during the mutator flush request do the
	 * Right Thing.
	 */
	@NoCheckStore
	...
	void assignThreadSlot() {
		if (!VM.runningVM) {
			...
			Magic.sync(); /*
							 * make sure that nobody sees the thread in any of
							 * the tables until the thread slot is inited
							 */
	...
	void releaseThreadSlot() {
		...
		threadIdx = -1;
		Magic.sync(); /*
						 * make sure that if someone is processing the threads
						 * array without holding the acctLock (which is
						 * definitely legal) then they see the replacementThread
						 * moved to the new index before they see the numThreads
						 * decremented (otherwise they would miss
						 * replacementThread; but with the current arrangement
						 * at worst they will see it twice)
						 */
	 ...
	 *
	 * @param stack
	 *            The stack on which to execute the thread.
	 * @param thread
	 *            The corresponding java.lang.Thread.
	 * @param name
	 *            The name of the thread
	 * @param daemon
	 *            True if this is a daemon thread.
	 * @param systemThread
	 *            True if this is a system thread.
	 * @param priority
	 *            The threads execution priority.
	 */
	public RVMThread(byte[] stack, Thread thread, String name, boolean daemon, SystemThread systemThread,
			int priority) {
		this.stack = stack;
		...
		if (!VM.runningVM) {
			...
			// Initialize the a thread stack as if "startoff" method had been
			// called
			// by an empty baseline-compiled "sentinel" frame with one local
			// variable.
	...
	public RVMThread(SystemThread systemThread, String name) {
		this(MemoryManager.newStack(STACK_SIZE_NORMAL), null, // java.lang.Thread
				name, true, // daemon
				systemThread, Thread.NORM_PRIORITY);
	}

	/**
	 * Create a thread with the given stack and name. Used by
	 * {@link org.jikesrvm.mm.mminterface.CollectorThread} and the boot image
	 * writer for the boot thread.
	 */
	...
	public RVMThread(Thread thread, long stacksize, String name, boolean daemon, int priority) {
		this(MemoryManager.newStack((stacksize <= 0) ? STACK_SIZE_NORMAL : (int) stacksize), thread, name, daemon, null,
				priority);
	}

	/**
	 * Check if the thread has block requests (for example, for suspension and
	 * GC). If it does, clear the requests and marked the thread as blocked for
	 * that request. If there were any block requests, do a broadcast() on the
	 * thread's monitor(). This is an internal method and should only be called
	 * from code that implements thread blocking. The monitor() lock must be
	 ...
	 * to be blocked. This will return true if the thread is actually blocking,
	 * or if the thread is running native code but is guaranteed to block before
	 * returning to Java. Only call this method when already holding the
	 ...
	 * blocked). So long as you hold the lock the thread cannot change state
	 ...
	 * method when already holding the monitor(), and probably only after
	 ...
	 * Heuristically, we use timer-based sampling the in the adaptive system to
	 * determine where the program is spending time (and thus what to optimize).
	 * This doesn't have to be a 100% accurate, but it must be non-blocking and
	 * also closely approximate whether or not the thread is executing. For now,
	 * approximate just as being in JAVA. As a future item, we may want to
	 * actually correctly attribute time spent in native code to the top native
	 * method on the frame when the timer goes off. This will require work in
	 * the JNI enter/exit sequence to deal with timer samples appropriately.
	 */
	...
	private void checkBlockNoSaveContext() {
		...
		if (commitSoftRendezvous) {
			// if a soft handshake had been requested, we need to acknowledge
			// it.
		...
		for (;;) {
			...
			if (traceReallyBlock) {
				...
				VM.sysWriteln("Thread #", threadSlot, " is really blocked with status ", getExecStatus());
	 ...
	 * method will ensure that soft handshake requests are acknowledged or else
	 ...
	 * once all blocking requests are cleared, and that other threads are
	 * notified that this thread is in the middle of blocking by setting the
	 * appropriate flag (<code>isBlocking</code>). Note that this thread
	 * acquires the monitor(), though it may release it completely either by
	 * calling wait() or by calling unlockCompletely(). Thus, although it isn't
	 * generally a problem to call this method while holding the monitor() lock,
	 * you should only do so if the loss of atomicity is acceptable.
	 * <p>
	 ...
	 * JDK. Why it's safe: the block() method needs to hold the monitor() for
	 * the time it takes it to make the block request, but does not need to
	 * continue to hold it when it calls checkBlock(). Thus, the fact that
	 * checkBlock() breaks atomicity is not a concern.
	 * <li>The yieldpoint. One of the purposes of a yieldpoint is to
	 * periodically check if the current thread should be blocked. This is
	 * accomplished by calling checkBlock(). Why it's safe: the yieldpoint
	 * performs several distinct actions, all of which individually require the
	 * monitor() lock - but the monitor() lock does not have to be held
	 * contiguously. Thus, the loss of atomicity from calling checkBlock() is
	 * fine.
	 * <li>The "WithHandshake" methods of HeavyCondLock. These methods allow you
	 * to block on a mutex or condition variable while notifying the system that
	 * you are not executing Java code. When these blocking methods return, they
	 * check if there had been a request to block, and if so, they call
	 * checkBlock(). Why it's safe: This is subtle. Two cases exist. The first
	 * case is when a WithHandshake method is called on a HeavyCondLock instance
	 * that is not a thread monitor(). In this case, it does not matter that
	 * checkBlock() may acquire and then completely release the monitor(), since
	 * the user was not holding the monitor(). However, this will break if the
	 * user is <i>also</i> holding the monitor() when calling the WithHandshake
	 * method on a different lock. This case should never happen because no
	 * other locks should ever be acquired when the monitor() is held.
	 * Additionally: there is the concern that some other locks should never be
	 * held while attempting to acquire the monitor(); the HeavyCondLock ensures
	 * that checkBlock() is only called when that lock itself is released. The
	 * other case is when a WithHandshake method is called on the monitor()
	 * itself. This should only be done when using <i>your own</i> monitor() -
	 * that is the monitor() of the thread your are running on. In this case,
	 * the WithHandshake methods work because: (i) lockWithHandshake() only
	 * calls checkBlock() on the initial lock entry (not on recursive entry), so
	 * atomicity is not broken, and (ii) waitWithHandshake() and friends only
	 * call checkBlock() after wait() returns - at which point it is safe to
	 * release and reacquire the lock, since there cannot be a race with
	 * broadcast() once we have committed to not calling wait() again.
	 * <li>Any code following a potentially-blocking native call. Case (3) above
	 * is somewhat subsumed in this except that it is special due to the fact
	 * that it's blocking on VM locks. So, this case refers specifically to JNI.
	 * The JNI epilogues will call leaveJNIBlocked(), which calls a variant of
	 * this method.
	 * </ol>
	 */
	@NoInline
	...
	/**
	 * Internal method for transitioning a thread from IN_JAVA or
	 * IN_JAVA_TO_BLOCK to either BLOCKED_IN_NATIVE or BLOCKED_IN_JNI, depending
	 * on the value of the jni parameter. It is always safe to conservatively
	 * call this method when transitioning to native code, though it is faster
	 * to call either enterNative(), enterJNIFromCallIntoNative(), or
	 * enterJNIFromJNIFunctionCall().
	 * <p>
	 * This method takes care of all bookkeeping and notifications required when
	 * a a thread that has been requested to block instead decides to run native
	 * code. Threads enter native code never need to block, since they will not
	 * be executing any Java code. However, such threads must ensure that any
	 * system services (like GC) that are waiting for this thread to stop are
	 * notified that the thread has instead chosen to exit Java. As well, any
	 * requests to perform a sot handshake must be serviced and acknowledged.
	 */
	...
	/**
	 * Called when JNI code tried to transition from IN_JNI to IN_JAVA but
	 * failed
	 */
	@Entrypoint
	@Unpreemptible("May block if the thread was asked to do so, but otherwise will not block")
	...
	private int setBlockedExecStatus() {
		...
		do {
			...
			/*
			 * use the CAS to assert that we observed what we thought we
			 * observed
			 */
	 ...
	 * the thread signals to us the intention to die as we are trying to block
	 * it, this will return TERMINATED. NOTE: the thread's execStatus will not
	 ...
	 * interruptible - so if you ask another thread to block and they ask you to
	 * block, you might deadlock.
	 */
	...
	int block(BlockAdapter ba, boolean asynchronous) {
		int result;
		if (traceBlock)
			VM.sysWriteln("Thread #", getCurrentThread().threadSlot, " is requesting that thread #", threadSlot,
					" blocks.");
		monitor().lockNoHandshake();
		...
		if (getCurrentThread() == this) {
			...
			if (isAboutToTerminate) {
				...
				result = newState;
				if (traceReallyBlock)
					VM.sysWriteln("Thread #", getCurrentThreadSlot(), " is blocking thread #", threadSlot,
							" which is in state ", newState);
				...
				// broadcast() happens _before_ the setting of the flags that
				// the
				// other threads would be awaiting, but that is fine, since
				// we're
				...
				if (newState == IN_JAVA_TO_BLOCK) {
					if (!asynchronous) {
						if (traceBlock)
							VM.sysWriteln("Thread #", getCurrentThread().threadSlot, " is waiting for thread #",
									threadSlot, " to block.");
						while (ba.hasBlockRequest(this, token) && !ba.isBlocked(this) && !isAboutToTerminate) {
							...
							if (VM.VerifyAssertions) {
								// do a timed wait, and assert that the thread
								// did not disappear
								// into native in the meantime
								monitor().timedWaitRelativeNoHandshake(1000L * 1000L * 1000L); // 1
																								// sec
								if (traceReallyBlock) {
									VM.sysWriteln("Thread #", threadSlot, "'s status is ", getExecStatus());
								}
					...
					// we own the thread for now - it cannot go back to
					// executing Java
					// code until we release the lock. before we do so we change
					// its
					// state accordingly and tell anyone who is waiting.
					if (traceBlock)
						VM.sysWriteln("Thread #", getCurrentThread().threadSlot, " has seen thread #", threadSlot,
		...
		monitor().unlock();
		if (traceReallyBlock)
			VM.sysWriteln("Thread #", getCurrentThread().threadSlot, " is done telling thread #", threadSlot,
					" to block.");
		return result;
	}
	...
	public void beginPairWith(RVMThread other) {
		...
		Monitor.lockWithHandshake(communicationLock(), Word.fromIntSignExtend(threadSlot), other.communicationLock(),
				Word.fromIntSignExtend(other.threadSlot));
	}
	...
	/**
	 * Save the current thread state. Call this prior to calling enterNative().
	 * You must be in a method that is marked BaselineSaveLSRegisters.
	 */
	@NoInline
	...
	/**
	 * Indicate that we'd like the current thread to be executing privileged
	 * code that does not require synchronization with the GC. This call may be
	 * made on a thread that is IN_JAVA or IN_JAVA_TO_BLOCK, and will result in
	 * the thread being either IN_NATIVE or BLOCKED_IN_NATIVE. In the case of an
	 * IN_JAVA_TO_BLOCK-&gt;BLOCKED_IN_NATIVE transition, this call will acquire
	 * the thread's lock and send out a notification to any threads waiting for
	 * this thread to reach a safepoint. This notification serves to notify them
	 * that the thread is in GC-safe code, but will not reach an actual
	 * safepoint for an indetermined amount of time. This is significant,
	 * because safepoints may perform additional actions (such as handling
	 * handshake requests, which may include things like mutator flushes and
	 * running isync) that IN_NATIVE code will not perform until returning to
	 * IN_JAVA by way of a leaveNative() call.
	 */
	...
	public static void enterNative() {
		...
		// NB this is not a correct assertion, as there is a race. we could
		// succeed in
		// CASing the status to IN_NATIVE, but then someone else could
		// asynchronosly
	...
	/**
	 * Attempt to transition from IN_JNI or IN_NATIVE to IN_JAVA, fail if
	 * execStatus is anything but IN_JNI or IN_NATIVE.
	 *
	...
	/**
	 * Leave privileged code. This is valid for threads that are either
	 * IN_NATIVE, IN_JNI, BLOCKED_IN_NATIVE, or BLOCKED_IN_JNI, and always
	 * results in the thread being IN_JAVA. If the thread was previously
	 * BLOCKED_IN_NATIVE or BLOCKED_IN_JNI, the thread will block until notified
	...
	public static void enterJNIFromCallIntoNative() {
		...
		if (!getCurrentThread().attemptFastExecStatusTransition(RVMThread.IN_JAVA, RVMThread.IN_JNI)) {
	...
	public static void leaveJNIFromCallIntoNative() {
		// FIXME: call these in PPC instead of doing it in machine code...
		getCurrentThread().observeExecStatus();
		if (!getCurrentThread().attemptFastExecStatusTransition(RVMThread.IN_JNI, RVMThread.IN_JAVA)) {
			RVMThread.leaveJNIBlockedFromCallIntoNative();
		}
	}

	public static void enterJNIFromJNIFunctionCall() {
		// FIXME: call these instead of doing it in machine code... currently
		// this
		// is never called.
		getCurrentThread().observeExecStatus();
		if (!getCurrentThread().attemptFastExecStatusTransition(RVMThread.IN_JAVA, RVMThread.IN_JNI)) {
	...
	public void unblock(BlockAdapter ba) {
		...
		monitor().unlock();
		if (traceBlock)
			VM.sysWriteln("Thread #", getCurrentThread().threadSlot, " is done requesting that thread #", threadSlot,
					" unblocks.");
	}
		...
		if (!isAboutToTerminate) {
			...
			if (isInJava()) {
				...
				if (contextRegisters != null) {
					...
					VM.sysWriteln(
							"(cannot dump stack trace; thread is not running in Java but has no contextRegisters)");
				}
			}
		}
		dumpLock.unlock();
	...
	public static void checkDebugRequest() {
		if (debugRequested) {
			...
			if (debugRequested) {
				...
				// FIXME: this code runs concurrently to GC and has no way of
				// stopping
				// it. hence it is dangerous. leaving it as-is for now, since
				// it's
	...
	/**
	 * Begin execution of current thread by calling its "run" method. This
	 * method is at the bottom of all created method's stacks.
	 */
	@Interruptible
	@SuppressWarnings({ "unused" })
	...
	private static void startoff() {
		...
		if (traceAcct) {
			VM.sysWriteln("Thread #", currentThread.threadSlot, " with pthread id ", currentThread.pthread_id,
					" running!");
		}
	...
	@Interruptible
	public void start() {
		// N.B.: cannot hit a yieldpoint between setting execStatus and starting
		// the
		// thread!!
		setExecStatus(IN_JAVA);
		acctLock.lockNoHandshake();
		...
		if (terminateSystem) {
			...
			/* Use System.exit so that any shutdown hooks are run. */ {
				...
				if (!mt.launched) {
					 ...
					 * initializers which were run by classes initialized before
					 * we attempted to run the main thread. (As of this writing,
					 * 24 January 2005, the Classpath libraries do not do such a
					 * thing, but there is no reason why we should not support
					 * this.) This was discussed on jikesrvm-researchers on 23
					 * Jan 2005 and 24 Jan 2005.
					 */
	...
	private void terminateUnpreemptible() {
		...
		if (cachedFreeLock != null) {
			if (Lock.trace) {
				VM.sysWriteln("Thread #", threadSlot, ": about to free lock ", Magic.objectAsAddress(cachedFreeLock));
			}
			if (VM.VerifyAssertions)
	...
	@NoOptCompile
	@NoInline
	// We should also have a pragma that saves all non-volatiles in opt
	// compiler,
	 ...
	 * The return barrier.
	 * <p>
	 * The following code implements return barriers as described for Lisp by
	 * Yuasa
	 *
	 ...
	 * <p>
	 * This code is executed when a method returns into a frame that has been
	 * hijacked by the return barrier mechanism. The return barrier trampoline
	 * will save state, execute this method, and then upon return from this
	 * method will transparently return into the frame that had been hijacked.
	 * <p>
	 * In this default implementation, the barrier reinstalls itself in the
	 * caller's frame thus incrementally moving the barrier down the stack.
	 * <p>
	 * The execution of this method is fragile. It is generally safest to call
	 * some other method from here that does the substantive work of the
	 * barrier.
	 */
	@Entrypoint
	@Uninterruptible
	...
	public static void returnBarrier() {
		...
		if (DEBUG_STACK_TRAMPOLINE) {
			VM.sysWriteln(getCurrentThread().getId(), " T0: ",
					getCurrentThread().trampolineRegisters.gprs.get(BaselineConstants.T0_int).toAddress());
			VM.sysWriteln(getCurrentThread().getId(), " T1: ",
					getCurrentThread().trampolineRegisters.gprs.get(BaselineConstants.T1_int).toAddress());
	...
	/**
	 * Install the stack trampoline bridge at a given frame, hijacking that
	 * frame, saving the hijacked return address and callee fp in thread-local
	 * state to allow execution of the hijacked frame later.
	 *
	 * @param targetFp
	 *            The frame to be hijacked.
	 */
	@Uninterruptible
	public void installStackTrampolineBridge(Address targetFp) {
		...
		if (trampoline.isZero()) {
			if (VM.VerifyAssertions)
				...
				VM.sysWriteln(
						"Warning: attempt to install stack trampoline without bridge instructions - nothing done.  See RVMThread.");
			 ...
			 */
			while (true) {
				if (Magic.getCallerFramePointer(targetFp)
						.EQ(ArchitectureSpecific.StackframeLayoutConstants.STACKFRAME_SENTINEL_FP)) {
					/*
					 * if we're at the bottom of the stack, then do not install
					 * anything
					 */
			...
			if (VM.VerifyAssertions)
				VM._assert(trampoline.NE(hijackedReturnAddress));
			if (DEBUG_STACK_TRAMPOLINE)
				dumpFrame(targetFp);
	...
	public void deInstallStackTrampoline() {
		if (DEBUG_STACK_TRAMPOLINE)
			VM.sysWriteln("deinstalling trampoline: ", framePointer);
		if (!hijackedReturnCalleeFp.isZero()) {
			if (DEBUG_STACK_TRAMPOLINE)
				VM.sysWriteln("need to reinstall: ", hijackedReturnAddress);
	...
	/**
	 * Given a frame that has been hijacked by the stack trampoline, return the
	 * real (hijacked) return address.
	 *
	 * @param hijackedFp
	...
	public static Address getHijackedReturnAddress(Address hijackedFp) {
		if (VM.VerifyAssertions)
			VM._assert(isTrampolineIP(Magic.getReturnAddressUnchecked(hijackedFp)));
		RVMThread t = getCurrentThread();
	...
	private static void dumpFrame(Address fp) {
		...
		while (sp.LE(nextFp)) {
			...
			if (sp.EQ(fp) || sp.EQ(nextFp))
				VM.sysWrite("* ");
			else if (sp.EQ(fp.plus(STACKFRAME_RETURN_ADDRESS_OFFSET))
					|| sp.EQ(nextFp.plus(STACKFRAME_RETURN_ADDRESS_OFFSET)))
				VM.sysWrite("R ");
			else if (sp.EQ(fp.plus(STACKFRAME_METHOD_ID_OFFSET)) || sp.EQ(nextFp.plus(STACKFRAME_METHOD_ID_OFFSET)))
				VM.sysWrite("M ");
			else
				VM.sysWrite(" ");
	...
	/**
	 * @return the caller of the frame in which the trampoline is installed
	 *         (STACKFRAME_SENTINEL_FP by default)
	 */
	public Address getNextUnencounteredFrame() {
		return hijackedReturnCallerFp.EQ(ArchitectureSpecific.StackframeLayoutConstants.STACKFRAME_SENTINEL_FP)
				? hijackedReturnCallerFp : Magic.getCallerFramePointer(hijackedReturnCallerFp);
	}
	...
	// We should also have a pragma that saves all non-volatiles in opt
	// compiler,
	// BaselineExecutionStateExtractor.java, should then restore all
	// non-volatiles
	...
	public void suspend() {
		if (false)
			VM.sysWriteln("Thread #", getCurrentThreadSlot(), " suspending Thread #", getThreadSlot());
		ObjectModel.genericUnlock(thread);
		Throwable rethrow = null;
		try {
			observeExecStatus();
			if (execStatus != IN_JAVA && execStatus != IN_JAVA_TO_BLOCK && execStatus != IN_NATIVE
					&& execStatus != BLOCKED_IN_NATIVE && execStatus != BLOCKED_IN_JNI && execStatus != IN_JNI) {
				throw new IllegalThreadStateException("Cannot suspend a thread that is not running.");
			}
	...
	public static void sleep(long ns) throws InterruptedException {
		...
		while (!t.hasInterrupt && t.asyncThrowable == null && sysCall.sysNanoTime() < whenEnd) {
	...
	void waitImpl(Object o, boolean hasTimeout, long whenWakeupNanos) {
		...
		if (asyncThrowable != null) {
			...
			if (l.waiting.isQueued(this)) {
				...
				// the lock, since acquiring the lock may require queueing the
				// thread.
	...
	/**
	 * Support for Java {@link java.lang.Object#wait()} synchronization
	 * primitive.
	 *
	 * @param o
	...
	public static void notify(Object o) {
		...
		if (l == null)
			return;
		// the reason for locking: when inflating a lock we *first* install it
		// in the status
		// word and *then* initialize its state. but fortunately, we do so while
		// holding
		// the lock's mutex. thus acquiring the lock's mutex is the only way to
		// ensure that
		...
		if (owner != me) {
			raiseIllegalMonitorStateException("notifying (expected lock to be held by " + me + "("
					+ getCurrentThread().getLockingId() + ") but was held by " + owner + "(" + l.getOwnerId() + ")) ",
					o);
		}
		l.mutex.lock();
	...
	public static void notifyAll(Object o) {
		...
		if (owner != getCurrentThread().getLockingId()) {
			raiseIllegalMonitorStateException("notifying all (expected lock to be held by "
					+ getCurrentThread().getLockingId() + " but was held by " + l.getOwnerId() + ") ", o);
		}
		for (;;) {
			l.mutex.lock();
			RVMThread toAwaken = l.waiting.dequeue();
	...
	public void park(boolean isAbsolute, long time) throws Throwable {
		...
		// massive retardation. someone might be holding the java.lang.Thread
		// lock.
	 ...
	 * thread's slot as returned by {@link #getThreadSlot()}, shifted
	 * appropriately so it can be directly used in the ownership tests.
	 */
	...
	public static class SoftHandshakeVisitor {
		 ...
		 * true. Returning true will cause a soft handshake request to be put
		 * through.
		 * <p>
		 * This method is called with the thread's monitor() held, but while the
		 * thread may still be running. This method is not called on mutators
		 * that have indicated that they are about to terminate.
		 */
		 ...
		 * running Java code. As such, it is safe to perform actions
	...
	public static int snapshotHandshakeThreads(SoftHandshakeVisitor v) {
		...
		acctLock.lockNoHandshake(); /*
									 * get a consistent view of which threads
									 * are live.
									 */
	 ...
	 * done so at least once. Additionally, call the visitor on each thread when
	 * making the yieldpoint request; the purpose of the visitor is to set any
	 * additional fields as needed to make specific requests to the threads that
	 * yield. Note that the visitor's <code>visit()</code> method is called with
	 * both the thread's monitor held, and the
	...
	public static void softHandshake(SoftHandshakeVisitor v) {
		handshakeLock.lockWithHandshake(); /*
											 * prevent multiple (soft or hard)
											 * handshakes from proceeding
											 * concurrently
											 */
		...
		for (int i = 0; i < numToHandshake; ++i) {
			...
			if (!t.isAboutToTerminate && v.checkAndSignal(t)) {
				...
				// Note that at this point if the thread tries to either enter
				// or
				...
				if (t.isInJava()) {
					// the thread is currently executing Java code, so we must
					// ensure
					// that it either:
					// 1) takes the next yieldpoint and rendezvous with this
					// soft
					// handshake request (see yieldpoint), or
					// 2) performs the rendezvous when leaving Java code
					// (see enterNativeBlocked, checkBlock, and
					// addAboutToTerminate)
					// either way, we will wait for it to get there before
					// exiting
					// this call, since the caller expects that after
					// softHandshake()
					// returns, no thread will be running Java code without
					// having
					// acknowledged.
					...
					// the thread is not in Java code (it may be blocked or it
					// may be
					// in native), so we don't have to wait for it since it will
					// do the Right Thing before returning to Java code.
					// essentially,
					// the thread cannot go back to running Java without doing
					// whatever
	...
	/**
	 * Commit the soft handshake rendezvous. This method cannot do anything that
	 * leads to a write barrier or allocation.
	 */
	 ...
	 * acknowledged the soft handshake. Thus - this is for actions in which it
	 * is sufficient for the thread to acknowledge that it plans to act upon the
	 * request in the immediate future, rather than that the thread acts upon
	 * the request prior to acknowledging.
	 * <p>
	 * This is almost always called with the monitor() lock held, but that's not
	 * guaranteed. If you need that lock, you can grab it (since it's a
	...
	void handleHandshakeRequest() {
		...
		if (VM.BuildForPowerPC && codePatchSyncRequested) {
			...
			// icache/dcache. --dave
			// A: Yes, this is sufficient. We (Filip and Dave) talked about it
			// and
	...
	/**
	 * Stop all mutator threads. This is current intended to be run by a single
	 * thread.
	 *
	 * Fixpoint until there are no threads that we haven't blocked. Fixpoint is
	 * needed to catch the (unlikely) case that a thread spawns another thread
	...
	public static void blockAllMutatorsForGC() {
		...
		while (true) {
			...
			// (2) Remove any threads that have already been blocked from the
			// list.
			for (int i = 0; i < numToHandshake; i++) {
				...
				if (t.blockedFor(RVMThread.gcBlockAdapter)
					...
					RVMThread.handshakeThreads[numToHandshake] = null; // help
																		// GC
				}
				t.monitor().unlock();
			}

			// (3) Quit trying to block threads if all threads are either
			// blocked
			// or not running (a thread is "not running" if it is NEW or
			// TERMINATED;
		...
		// Deal with terminating threads to ensure that all threads are either
		// dead to MMTk or stopped above.
	...
	public static final AllButGCHardHandshakeVisitor allButGC = new AllButGCHardHandshakeVisitor();
	...
	public static void hardHandshakeSuspend(BlockAdapter ba, HardHandshakeVisitor hhv) {
		...
		processAboutToTerminate(); /*
									 ...
									 * we were stopping the world notify us that
									 * they had stopped.
									 */
	...
	public static void yieldpoint(int whereFrom, Address yieldpointServiceMethodFP) {
		...
		t.atYieldpoint = true;
		t.yieldpointsTaken++;
		// If thread is in critical section we can't do anything right now,
		// defer
		// until later
		// we do this without acquiring locks, since part of the point of
		// disabling
		...
		// since we are guaranteeing that a yieldpoint will run after we emerge
		// from
		// the no-yieldpoints code. At worst, setting takeYieldpoint to 0 will
		// be
		// lost (because some other thread sets it to non-0), but in that case
		// we'll
		...
		if (takeYieldpointVal != 0) {
			...
			if (t.timeSliceExpired != 0) {
				...
				if (t.yieldForCBSCall || t.yieldForCBSMethod) {
					/*
					 * CBS Sampling is still active from previous quantum. Note
					 * that fact, but leave all the other CBS parameters alone.
					 */
				} else {
			...
			// thread switch was desired for other reasons, then we need to
			// ensure
			// that between when this runs and when the glue code runs there
			// will
	 ...
	 *            register state at which stack overflow trap was encountered
	 *            (null --> normal method call, not a trap)
	 */
	...
	@NoInline
	@BaselineNoRegisters
	// this method does not do a normal return and hence does not execute
	// epilogue
	...
	public void fixupMovedStack(Offset delta) {
		...
		if ((exceptionRegisters.inuse) && (exceptionRegisters.getInnermostFramePointer().NE(Address.zero()))) {
			adjustRegisters(exceptionRegisters, delta);
		}
		if (!contextRegisters.getInnermostFramePointer().isZero()) {
	...
	/**
	 * A thread's stack has been moved or resized. Adjust registers to reflect
	 * new position.
	 *
	 * @param registers
	 ...
	 * only have setDaemon invoked on it before it is started, Threads can
	 * become daemons at any time. Note: making the last non daemon a daemon
	 * will terminate the VM.
	 * <p>
	 * Note: This method might need to be uninterruptible so it is final, which
	 * is why it isn't called setDaemon.
	 * <p>
	...
	public void join(long ms, int ns) throws InterruptedException {
		...
		// another thread's lock using the WithHandshake scheme, as that would
		// result
	...
	public void handleUncaughtException(Throwable exceptionObject) {
		...
		VM.enableGC();
		if (thread == null) {
			VM.sysWrite("Exception in the primordial thread \"", getName(), "\" while booting: ");
		} else {
	...
	private void handlePossibleRecursiveException() {
		if (uncaughtExceptionCount > 1 && uncaughtExceptionCount <= VM.maxSystemTroubleRecursionDepth
	 ...
	 *
	 * @param leftJustify
	 *            minimum number of characters emitted, with any extra
	...
	public int dump(char[] dest, int offset) {
		...
		if (daemon) {
			offset = Services.sprintf(dest, offset, "-daemon"); // daemon
																// thread?
		}
		if (isBootThread()) {
			offset = Services.sprintf(dest, offset, "-Boot"); // Boot
																// (Primordial)
			// thread
		}
		if (isSystemThread()) {
	 ...
	 * numActiveThreads d* = numActiveDaemons * parenthetical values, printed
	 * only if traceDetails = true)
	 * <p>
	 ...
	 * <p>
	 * Note: the system could be in a fragile state when this method is called,
	 * so we try to rely on as little runtime functionality as possible (eg. use
	 * no bytecodes that require RuntimeEntrypoints support).
	 */
	...
	public static void dumpStack() {
		if (VM.runningVM) {
			...
			StackTraceElement[] elements = (new Throwable("--traceback from Jikes RVM's RVMThread class--"))
					.getStackTrace();
	 ...
	 * <li>Immortal (For threads allocated in immortal space such as collectors)
	 * </li>
	 ...
	 * STACKFRAME_SENTINEL_FP is possible when the thread has been created but
	...
	private static void showMethod(int compiledMethodId, Address fp) {
		showPrologue(fp);
		VM.sysWrite("<unprintable normal Java frame: CompiledMethods.getCompiledMethod(", compiledMethodId,
				") returned null>\n");
	}
	 ...
	 *
	 * @param fp
	 *            address of starting frame Returned: doesn't return. This
	 *            method is called from RunBootImage.C when something goes
	 *            horrifically wrong with exception handling and we want to die
	 *            with useful diagnostics.
	 */
	@Entrypoint
	public static void dumpStackAndDie(Address fp) {
		if (!exitInProgress) {
			// This is the first time I've been called, attempt to exit
			// "cleanly"
			exitInProgress = true;
			dumpStack(fp);
	...
	public static RVMThread setupBootThread() {
		...
		bootThread.feedlet = TraceEngine.engine.makeFeedlet("Jikes RVM boot thread",
	...
	static final int[] sloppyExecStatusHistogram = new int[LAST_EXEC_STATUS];
	static final int[] statusAtSTWHistogram = new int[LAST_EXEC_STATUS];
	static final int[] execStatusTransitionHistogram = new int[LAST_EXEC_STATUS * LAST_EXEC_STATUS];
	...
	public static void reportThreadTransitionCounts() {
		...
		for (int fromI = 0; fromI < LAST_EXEC_STATUS; ++fromI) {
			for (int toI = 0; toI < LAST_EXEC_STATUS; ++toI) {
				int val = execStatusTransitionHistogram[transitionHistogramIndex(fromI, toI)];
				if (val != 0) {
