...
public final class MainThread extends Thread {
	 ...
	 * 
	 * @param args
	 *            {@code args[0]}: name of class containing "main" method;
	 *            {@code args[1..N]}: parameters to pass to "main" method
	 */
	public MainThread(String[] args) {
		super("MainThread");
		setDaemon(false); // NB otherwise we inherit the boot threads daemon
							// status
	...
	private void runAgents(ClassLoader cl) {
		if (agents.length > 0) {
			...
			for (String agent : agents) {
				/*
				 * Parse agent string according to the form given in the
				 * java.lang.instrumentation package documentation:
				 * jarpath[=options]
				 *
				 * (The -javaagent: part of the agent options has already been
				 * stripped)
				 */
	...
	private static void runAgent(Instrumentation instrumenter, ClassLoader cl, String agentJar, String agentOptions) {
		...
		// TODO: By this stage all agent jars and classes they reference via
		// their manifest
		try {
			Class<?> agentClass = cl.loadClass(agentClassName);
			Method agentPremainMethod = agentClass.getMethod("premain",
	 ...
	 * Run "main" thread.
	 * <p>
	 * This code could be made a little shorter by relying on Reflection to do
	 * the classloading and compilation. We intentionally do it here to give us
	 * a chance to provide error messages that are specific to not being able to
	 * find the main class the user wants to run. This may be a little silly,
	 * since it results in code duplication just to provide debug messages in a
	 * place where very little is actually likely to go wrong, but there you
	 * have it....
	 */
	@Override
	@Entrypoint
