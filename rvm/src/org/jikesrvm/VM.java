...
public class VM extends Properties implements Constants, ExitStatus {
	...
	private static Thread coffThread;
	 ...
	 * 
	 * @param classPath
	 *            class path to be used by RVMClassLoader
	 * @param bootCompilerArgs
	 *            command line arguments for the bootimage compiler
	 */
	@Interruptible
	...
	public static void initForTool(String classpath) {
		if (VM.VerifyAssertions)
			VM._assert(!VM.runningVM);
		if (VM.VerifyAssertions)
			VM._assert(!VM.writingBootImage);
		runningTool = true;
		init(classpath, null);
	}
	 ...
	 * <p>
	 *
	 * Uninterruptible because we are not setup to execute a yieldpoint or
	 * stackoverflow check in the prologue this early in booting.
	 * <p>
	 *
	 * The following machine registers are set by "C" bootstrap program before
	 * calling this method:
	 * <ol>
	 * <li>JTOC_POINTER - required for accessing globals
	 * <li>FRAME_POINTER - required for accessing locals
	 * <li>THREAD_ID_REGISTER - required for method prolog (stack overflow
	 * check)
	 * </ol>
	 */
	...
	public static void boot() {
		...
		// The "stackLimit" must be set before any interruptible methods are
		// called
		...
		currentThread.stackLimit = Magic.objectAsAddress(currentThread.getStack())
				.plus(ArchitectureSpecific.StackframeLayoutConstants.STACK_SIZE_GUARD);

		finishBooting();
	}

	/**
	 * Complete the task of booting Jikes RVM. Done in a secondary method mainly
	 * because this code doesn't have to be uninterruptible and this is the
	 * cleanest way to make that distinction.
	 */
	@Interruptible
	private static void finishBooting() {
		...
		if (VM.BuildForGnuClasspath) {
			runClassInitializer("java.util.WeakHashMap"); // Need for
															// ThreadLocal
		}
		...
		/*
		 * We can safely allocate a java.lang.Thread now. The boot thread
		 * (running right now, as a Thread) has to become a full-fledged Thread,
		 * since we're about to encounter a security check:
		 * 
		 * EncodingManager checks a system property, which means that the
		 * permissions checks have to be working, which means that
		 * VMAccessController will be invoked, which means that
		 * ThreadLocal.get() will be called, which calls
		...
		// Among other things, after this returns, GC and dynamic class loading
		// are enabled.
		//
		if (verboseBoot >= 1)
		...
		runClassInitializer("java.io.File"); // needed for when we initialize
												// the
		 ...
		 * means that we need a working -jar before we can return an Application
		 * Class Loader.
		 */
		...
		// Initialize java.lang.System.out, java.lang.System.err,
		// java.lang.System.in
	...
	/**
	 * Run {@code <clinit>} method of specified class, if that class appears in
	 * bootimage and actually has a clinit method (we are flexible to allow one
	 * list of classes to work with different bootimages and different version
	 * of classpath (eg 0.05 vs. cvs head).
	 * <p>
	...
	static void runClassInitializer(String className) {
		...
		TypeReference tRef = TypeReference.findOrCreate(BootstrapClassLoader.getBootstrapClassLoader(),
				classDescriptor);
		...
		if (null == cls) {
			...
			if (clinit != null) {
				...
				// <clinit> is no longer needed: reclaim space by removing
				// references to it
	...
	/**
	 * Verify a runtime assertion (die w/message and traceback if assertion
	 * fails).
	 * <p>
	 *
	 * Note: code your assertion checks as
	 ...
	 *            the assertion to verify
	 * @param message
	 *            the message to print if the assertion is false
	 */
	...
	/**
	 * Format a 32 bit number as "0x" followed by 8 hex digits. Do this without
	 * referencing Integer or Character classes, in order to avoid dynamic
	 * linking. TODO: move this method to Services.
	 * 
	 * @param number
	...
	/**
	 * Format a 64 bit number as "0x" followed by 16 hex digits. Do this without
	 * referencing Long or Character classes, in order to avoid dynamic linking.
	...
	/**
	 * Format a 32/64 bit number as "0x" followed by 8/16 hex digits. Do this
	 * without referencing Integer or Character classes, in order to avoid
	 * dynamic linking. TODO: move this method to Services.
	 * 
	 * @param addr
	 ...
	 * 
	 * @param value
	 *            character array that is printed
	 * @param len
	...
	public static void write(int value) {
		if (runningVM) {
			int mode = (value < -(1 << 20) || value > (1 << 20)) ? 2 : 0; // hex
																			// only
																			// or
																			// decimal
																			// only
	 ...
	 *            what is printed
	 * @param hexToo
	 *            how to print: true - print as decimal followed by hex false -
	 ...
	 * 
	 * @param value
	 *            print value and left-fill with enough spaces to print at least
	 ...
	 * 
	 * @param fieldWidth
	 *            Minimum width to print.
	 * @param s
	 *            The {@link Atom} to print.
	 */
	@NoInline
	/* don't waste code space inlining these --dave */
	...
	/**
	 * A group of multi-argument sysWrites with optional newline. Externally
	 * visible methods.
	 */
	@NoInline
	...
	@NoInline
	public static void sysWriteln(String s0, Address a1, String s1, Word w1, String s2, int i1, String s3, int i2,
			String s4, Word w2, String s5, int i3) {
		swLock();
		write(s0);
		write(a1);
		write(s1);
	 ...
	 *            error message describing the problem
	 * @param number
	 *            an integer to append to <code>message</code>.
	 */
	@NoInline
	...
	/**
	 * Shut down the virtual machine. Should only be called if the VM is
	 * running.
	 * 
	 * @param value
	 *            exit value
	 ...
	 *            "shutdown".
	 * @param depth
	 *            How deep are we in that function?
	 * @param message
	 *            What message did it have? null means this particular shutdown
	 *            function does not come with a message.
	 * @param showNumber
	 *            Print <code>number</code> following <code>message</code>?
	 * @param number
	 *            Print this number, if <code>showNumber</code> is {@code true}.
	 */
	private static void handlePossibleRecursiveExit(String called, int depth, String message, boolean showNumber,
			int number) {
		if (depth > 1 && (depth <= maxSystemTroubleRecursionDepth
				+ VM.maxSystemTroubleRecursionDepthBeforeWeStopVMSysWrite)) {
			if (showNumber) {
				tsysWriteln("VM.", called, "(): We're in a", " (likely)", " recursive call to VM.", called, "(), ",
						depth, " deep\n", message == null ? "" : "   ", message == null ? "" : called,
						message == null ? "" : " was called with the message: ", message == null ? "" : message,
						number);
			} else {
				tsysWriteln("VM.", called, "(): We're in a", " (likely)", " recursive call to VM.", called, "(), ",
						depth, " deep\n", message == null ? "" : "   ", message == null ? "" : called,
	...
	/**
	 * Have we already called dieAbruptlyRecursiveSystemTrouble()? Only for use
	 * if we're recursively shutting down! Used by
	...
	public static void dieAbruptlyRecursiveSystemTrouble() {
		...
		/*
		 * And if THAT fails, go into an infinite loop. Ugly, but it's better
		 * than returning from this function and leading to yet more cascading
		 * errors. and misleading error messages. (To the best of my knowledge,
		 * we have never yet reached this point.)
		 */
		while (true) {
			;
		}
	...
	/**
	 * Create class instances needed for boot image or initialize classes needed
	 * by tools.
	 * 
	 * @param bootstrapClasspath
	 *            places where VM implementation class reside
	 * @param bootCompilerArgs
	 *            command line arguments to pass along to the boot compiler's
	 *            init routine.
	 */
	@Interruptible
	 ...
	 * allocate objects explicitly via "new", or implicitly via methods that, in
	 * turn, call "new" (such as string concatenation expressions that are
	 ...
	 * such as:
	 * <ul>
	 * <li>calling methods or accessing fields of classes that haven't yet been
	 ...
	 * match exactly
	 * <li>typecasting objects, except when runtime types of lhs & rhs match
	 * exactly
	 * </ul>
	 *
	 * <p>
	 * Recommendation: as a debugging aid, Allocator implementations should test
	 * "Thread.disallowAllocationsByThisThread" to verify that they are never
	...
	/**
	 * disableGC: Disable GC if it hasn't already been disabled. This enforces a
	 * stack discipline; we need it for the JNI Get*Critical and
	...
	public static void disableGC(boolean recursiveOK) {
		// current (non-GC) thread is going to be holding raw addresses,
		// therefore we must:
		//
		...
		if (Magic.getFramePointer().minus(ArchitectureSpecific.StackframeLayoutConstants.STACK_SIZE_GCDISABLED)
			...
			RVMThread.resizeCurrentStack(
					myThread.getStackLength() + ArchitectureSpecific.StackframeLayoutConstants.STACK_SIZE_GCDISABLED,
					null);
		}
		...
		if (VM.VerifyAssertions) {
			if (!recursiveOK) {
				VM._assert(!myThread.getDisallowAllocationsByThisThread()); // recursion
																			// not
																			// allowed
			}
	...
	/**
	 * enableGC(): Re-Enable GC if we're popping off the last possibly-recursive
	 * {@link #disableGC} request. This enforces a stack discipline; we need it
	 * for the JNI Get*Critical and Release*Critical functions. Should be
	 * matched with a preceding call to {@link #disableGC}.
	 */
	@Inline
	...
	/**
	 * Is this a build for 32bit addressing? NB. this method is provided to give
	 * a hook to the IA32 assembler that won't be compiled away by javac
	 */
	...
	/**
	 * Is this a build for SSE2? NB. this method is provided to give a hook to
	 * the IA32 assembler that won't be compiled away by javac
	 */
