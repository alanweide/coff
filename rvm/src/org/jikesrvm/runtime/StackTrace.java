...
public class StackTrace {
	/**
	 * The compiled method ids of the stack trace. Ordered with the top of the
	 * stack at 0 and the bottom of the stack at the end of the array
	 */
	...
	/**
	 * Index of the last stack trace; only used to support
	 * VM.VerboseStackTracePeriod
	 */
	...
	/**
	 * Walk the stack counting the number of stack frames encountered. The stack
	 * being walked is our stack, so code is Uninterruptible to stop the stack
	 * moving.
	 * 
		...
		while (Magic.getCallerFramePointer(fp).NE(STACKFRAME_SENTINEL_FP)) {
			...
			if (compiledMethodId != INVISIBLE_METHOD_ID) {
				...
				if ((compiledMethod.getCompilerType() != CompiledMethod.TRAP)
					...
					// skip native frames, stopping at last native frame
					// preceeding the
	...
	/**
	 * Walk the stack recording the stack frames encountered The stack being
	 * walked is our stack, so code is Uninterrupible to stop the stack moving.
	 */
	@Uninterruptible
	@NoInline
				...
				if ((compiledMethod == null) || (compiledMethod.getCompilerType() != CompiledMethod.OPT)) {
					...
					if (iei < 0) {
						...
						for (; iei >= 0; iei = OptEncodedCallSiteTree.getParent(iei, inlineEncoding)) {
							...
							RVMMethod method = MemberReference.getMemberRef(mid).asMethodReference()
									.getResolvedMember();
	 ...
	 * 
	 * @param first
	 *            the first compiled method to look from
	 * @param last
	 *            the last compiled method to look to
	 */
	...
	private int firstRealMethod(Throwable cause) {
		/*
		 * We expect a hardware trap to look like: at
		 * org.jikesrvm.runtime.StackTrace.<init>(StackTrace.java:78) at
		 * java.lang.VMThrowable.fillInStackTrace(VMThrowable.java:67) at
		 * java.lang.Throwable.fillInStackTrace(Throwable.java:498) at
		 * java.lang.Throwable.<init>(Throwable.java:159) at
		 * java.lang.Throwable.<init>(Throwable.java:147) at
		 * java.lang.Exception.<init>(Exception.java:66) at
		 * java.lang.RuntimeException.<init>(RuntimeException.java:64) at
		 * java.lang.NullPointerException.<init>(NullPointerException.java:69)
		 * at org.jikesrvm.runtime.RuntimeEntrypoints.deliverHardwareException(
		 * RuntimeEntrypoints.java:682) at <hardware trap>(Unknown Source:0)
		 *
		 * and a software trap to look like: at
		 * org.jikesrvm.runtime.StackTrace.<init>(StackTrace.java:78) at
		 * java.lang.VMThrowable.fillInStackTrace(VMThrowable.java:67) at
		 * java.lang.Throwable.fillInStackTrace(Throwable.java:498) at
		 * java.lang.Throwable.<init>(Throwable.java:159) at
		 * java.lang.Error.<init>(Error.java:81) at
		 * java.lang.LinkageError.<init>(LinkageError.java:72) at
		 * java.lang.ExceptionInInitializerError.<init>(
		 * ExceptionInInitializerError.java:85) at
		 * java.lang.ExceptionInInitializerError.<init>(
		 * ExceptionInInitializerError.java:75)
		 *
		 * and an OutOfMemoryError to look like: ??? ... at
		 * org.jikesrvm.mm.mminterface.MemoryManager.allocateSpace(MemoryManager
		 * .java:613) ... at
		 * org.jikesrvm.runtime.RuntimeEntrypoints.unresolvedNewArray(
		 * RuntimeEntrypoints.java:401)
		 */
		if (Options.stackTraceFull) {
			...
			if (cause instanceof OutOfMemoryError) {
				...
				while ((element < compiledMethods.length) && (compiledMethod != null) && compiledMethod.getMethod()
						.getDeclaringClass().getClassForType() != RuntimeEntrypoints.class) {
					element++;
			...
			while ((element < compiledMethods.length) && (compiledMethod != null)
			...
			if (VM.BuildForGnuClasspath) {
				while ((element < compiledMethods.length) && (compiledMethod != null) && compiledMethod.getMethod()
						.getDeclaringClass().getClassForType().getName().equals("java.lang.VMThrowable")) {
					element++;
					compiledMethod = getCompiledMethod(element);
				}
			}
			...
			// (4) remove frames belonging to exception constructors upto the
			// causes constructor
			while ((element < compiledMethods.length) && (compiledMethod != null)
			...
			// exception is thrown from a constructor of the cause exception,
			// however,
	 ...
	 * 
	 * @param first
	 *            the first real method of the stack trace
	 * @return compiledMethods.length-1 if no non-VM methods found else the
	 *         index of the method
	 */
	private int lastRealMethod(int first) {
		/*
		 * We expect an exception on the main thread to look like: at <invisible
		 * method>(Unknown Source:0) at
		 * org.jikesrvm.runtime.Reflection.invoke(Reflection.java:132) at
		 * org.jikesrvm.scheduler.MainThread.run(MainThread.java:195) at
		 * org.jikesrvm.scheduler.RVMThread.run(RVMThread.java:534) at
		 * org.jikesrvm.scheduler.RVMThread.startoff(RVMThread.java:1113
		 *
		 * and on another thread to look like: at
		 * org.jikesrvm.scheduler.RVMThread.run(RVMThread.java:534) at
		 * org.jikesrvm.scheduler.RVMThread.startoff(RVMThread.java:1113)
		 */
		...
		if (Options.stackTraceFull) {
			...
			// Start at end of array and elide a frame unless we find a place to
			// stop
			for (int i = max; i >= first; i--) {
				if (compiledMethods[i] == INVISIBLE_METHOD_ID) {
					// we found an invisible method, assume next method if this
					// is sane
					if (i - 1 >= 0) {
						return i - 1;
					} else {
