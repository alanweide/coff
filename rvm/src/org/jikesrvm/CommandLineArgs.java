...
public class CommandLineArgs {
	...
	private enum PrefixType {
		...
		// -----------------------------------------------//
		CLASSPATH_ARG, ENVIRONMENT_ARG, VERBOSE_JNI_ARG, VERBOSE_CLS_ARG, JAR_ARG, JAVAAGENT_ARG, ENABLE_ASSERTION_ARG, ENABLE_SYSTEM_ASSERTION_ARG, DISABLE_ASSERTION_ARG, DISABLE_SYSTEM_ASSERTION_ARG,

		// -----------------------------------------------//
		// The following arguments are RVM-specific. //
		// -----------------------------------------------//
		HELP_ARG, ARG, IRC_HELP_ARG, IRC_ARG, RECOMP_HELP_ARG, RECOMP_ARG, AOS_HELP_ARG, AOS_ARG, BASE_HELP_ARG, BASE_ARG, OPT_ARG, OPT_HELP_ARG,
		/* Silently ignored */
		VERIFY_ARG, GC_HELP_ARG, GC_ARG, BOOTCLASSPATH_P_ARG, BOOTCLASSPATH_A_ARG, BOOTSTRAP_CLASSES_ARG, AVAILABLE_PROCESSORS_ARG,
	...
	private static final class Prefix implements Comparable<Prefix> {
		 ...
		 *            argument string
		 * @param t
		 *            type of prefix, must be non-null
		 */
	...
	/**
	 * A list of possible prefixes for command line arguments. Each argument
	 * will be classified by the prefix it matches. One prefix can contain
	 * another.
	 * <p>
	 *
	 * Prefixes are normally matched with the start of the argument. If the last
	 * character of the prefix is a '$', the prefix (without the trailing '$')
	 * will be matched with the whole argument. If the last character of the
	 * prefix is a ' ' (space), the prefix (without the trailing ' ') will be
	 * matched with the whole argument, and the next argument will be appended
	 * to the end of the one matching the prefix, with a space in between.
	 * <p>
	 *
	 * The type will be used to classify the prefix. Multiple entries CAN have
	 * the same type.
	 */
	private static final Prefix[] prefixes = { new Prefix("-classpath ", PrefixType.CLASSPATH_ARG),
			...
			// Note: space is significant
			new Prefix("-javaagent:", PrefixType.JAVAAGENT_ARG), new Prefix("-D", PrefixType.ENVIRONMENT_ARG),
			...
			new Prefix("-X:irc:help$", PrefixType.IRC_HELP_ARG), new Prefix("-X:irc$", PrefixType.IRC_HELP_ARG),
			new Prefix("-X:irc:", PrefixType.IRC_ARG), new Prefix("-X:recomp:help$", PrefixType.RECOMP_HELP_ARG),
			new Prefix("-X:recomp$", PrefixType.RECOMP_HELP_ARG), new Prefix("-X:recomp", PrefixType.RECOMP_ARG),
			new Prefix("-X:aos:help$", PrefixType.AOS_HELP_ARG), new Prefix("-X:aos$", PrefixType.AOS_HELP_ARG),
			new Prefix("-X:aos:", PrefixType.AOS_ARG), new Prefix("-X:gc:help$", PrefixType.GC_HELP_ARG),
			new Prefix("-X:gc$", PrefixType.GC_HELP_ARG), new Prefix("-X:gc:", PrefixType.GC_ARG),
			new Prefix("-X:base:help$", PrefixType.BASE_HELP_ARG), new Prefix("-X:base$", PrefixType.BASE_HELP_ARG),
			new Prefix("-X:base:", PrefixType.BASE_ARG), new Prefix("-X:opt:help$", PrefixType.OPT_HELP_ARG),
			new Prefix("-X:opt$", PrefixType.OPT_HELP_ARG), new Prefix("-X:opt:", PrefixType.OPT_ARG),
			new Prefix("-X:vm:help$", PrefixType.HELP_ARG), new Prefix("-X:vm$", PrefixType.HELP_ARG),
	...
	static void fetchCommandLineArguments() {
		...
		for (int i = 0; i < numArgs; ++i) {
			...
			for (Prefix p : prefixes) {
				...
				// Chop off the prefix (which we've already matched) and store
				// the
				// value portion of the string (the unique part) in args[i].
				// Store
				...
				arg_types[i] = p.type;
				p = findPrefix(p.type); // Find the canonical prefix for this
										// type...
	...
	/**
	 * Extract all command line arguments of a particular type. Strips out the
	 * prefixes (if any). !!TODO: cache results instead of constructing a new
	 * array each time.
	 * 
	 * @param prefix
	...
	public static String[] getEnvironmentArgs() {
		if (!VM.runningVM)
			throw new IllegalAccessError("Environment variables can't be read in a non-running VM");
	...
	/**
	 * Stage1 processing of virtual machine directives appearing in argument
	 * list. We try to process as many classes of command line arguments as
	 * possible here. Only those command line arguments that require a more or
	 * less fully booted VM to handle are delayed until
	...
	static void earlyProcessCommandLineArguments() {
		for (int i = 0; i < app_name_pos; i++) {
			...
			if (type == PrefixType.INVALID_ARG)
				continue;
			Prefix p = findPrefix(type);
			if (DEBUG)
				VM.sysWriteln(" CommandLineArgs.earlyProcessCLA(" + p + arg + " - " + type + ")");
			switch (type) {

			case CLASSPATH_ARG:
				// arguments of the form "-classpath a:b:c" or "-cp a:b:c"
				// We are experimentally processing this early so that we can
				// have the
			...
			// talking to several different compilers used by AOS for
			// recompilation.
			// --------------------------------------------------------------------
			case RECOMP_HELP_ARG:
				if (VM.BuildForAdaptiveSystem) {
					...
					VM.sysWriteln(
							"vm: nonadaptive configuration; -X:recomp is not a legal prefix in this configuration");
					VM.sysExit(VM.EXIT_STATUS_BOGUS_COMMAND_LINE_ARG);
				}
				break;
			case RECOMP_ARG:
	...
	/**
	 * Stage2 processing of virtual machine directives appearing in argument
	 * list. This function is responsible for processing the few command line
	 * arguments that need to be handled late in booting. It also returns the
	 * application's command line arguments.
	 *
	 * @return application arguments (first is application class name) If no
	 *         application arguments are specified on the command line, process
	 *         commands anyway.
	 */
	static String[] lateProcessCommandLineArguments() {
		for (int i = 0; i < app_name_pos; i++) {
			...
			Prefix p = findPrefix(type);
			if (DEBUG)
				VM.sysWriteln(" CommandLineArgs.processCLA(" + p + arg + " - " + type + ")");
			switch (type) {
	 ...
	 * 
	 * @param argno
	 *            Number of argument sought
	 * @param buf
	 *            Buffer to fill
	 * @return number of bytes placed in buffer. -1 means buffer too small for
	 *         argument to fit)
	 */
	...
	/**
	 * Primitive parsing of float/double values. Done this way to enable us to
	 * parse command line arguments early in VM booting before we are able to do
	 * the JNI call that using {@code Double.valueOf} would require. Does not
	 * support the full Java specification.
	 * 
	 * @param arg
	...
	/**
	 * Primitive parsing of byte/integer numeric values. Done this way to enable
	 * us to parse command line arguments early in VM booting before we are able
	 * call {@code  Byte.parseByte} or {@code Integer.parseInt}.
	 * 
	 * @param arg
	 ...
	 *
	 * @return Negative values on error. Otherwise, positive or zero values as
	 *         bytes.
	 */
	public static long parseMemorySize(String sizeName, String sizeFlag, String defaultFactor, int roundTo,
		...
		return sysCall.sysParseMemorySize(s2b(sizeName), s2b(sizeFlag), s2b(defaultFactor), roundTo, s2b(fullArg),
				s2b(subArg));
	}
	...
	private static final class ArgReader {
		...
		/**
		 * Read argument # @param i Assume arguments are encoded in the
		...
		String getArg(int i) {
			 ...
			 *
			 * Instead, we use the four-argument constructor, the one that takes
			 * a HIBYTE parameter.
			 *
			 * 1) It is safe to do this; we *know* that all of the legal
			 * command-line args use only characters within the ASCII character
			 * set.
			 *
	 ...
	 * This is used for converting arguments and for converting fixed strings we
	 * pass down to lower commands.
	 *
	 * @param arg
	 ...
	 *
	 * @return a byte array that represents <code>arg</code> as a {@code null}
	 *         -terminated C string. Returns {@code null} for a {@code null}
	 *         arg.
	 */
